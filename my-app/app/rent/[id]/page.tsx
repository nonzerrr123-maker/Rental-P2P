  ...
  return <main className="min-h-screen bg-gray-50 text-black">
    ...
    <div className="mx-auto grid max-w-6xl gap-8 px-6 py-12 lg:grid-cols-2">
      ...
      <section className="rounded-2xl border bg-white p-8">
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2"><label className="text-sm font-semibold">วันเริ่มยืม<input type="date" value={start} onChange={(e) => setStart(e.target.value)} min={new Date().toISOString().slice(0,10)} className="mt-2 w-full rounded-lg border px-3 py-3 font-normal" /></label><label className="text-sm font-semibold">วันคืน<input type="date" value={end} onChange={(e) => setEnd(e.target.value)} min={start || new Date().toISOString().slice(0,10)} className="mt-2 w-full rounded-lg border px-3 py-3 font-normal" /></label></div>
        ...
        {requested ? <div className="mt-5 rounded-xl border border-[#D4AF37] bg-[#fffaf0] p-5"><b>ส่งคำขอยืมแล้ว ✓</b><p className="mt-1 text-sm text-gray-600">สถานะ: PENDING · รอผู้ให้ยืมตอบรับ คุณสามารถพูดคุยรายละเอียดผ่านแชตได้</p><a href="/chat" className="mt-3 inline-block font-bold text-[#9b7b12]">เปิดแชต →</a></div> : <button disabled={!days} onClick={() => setRequested(true)} className="mt-6 w-full rounded-lg bg-[#D4AF37] px-5 py-4 font-black text-black enabled:hover:bg-[#e5c04a] disabled:cursor-not-allowed disabled:opacity-40">{days ? `ขอยืม ${days} วัน · ฿${total.toLocaleString()}` : "เลือกวันยืมและวันคืนก่อน"}</button>}
        ...
        <p className="mt-4 text-center text-xs text-gray-400">Prototype: ก่อนส่งคำขอจริง ระบบ production ต้องตรวจ VERIFIED จาก server และตรวจ availability ของช่วงวันที่เลือก</p>
      </section>
    </div>
  </main>;
}
